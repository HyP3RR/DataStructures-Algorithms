...
int main() {
    ...
    for (int i = 0; i < m; i++) {
        ...
        u--;
        v--;
        if (find_representative(u) != find_representative(v)) {
            components--;
            union_set(u, v);
            size_largest = max(size_largest, set_size[find_representative(u)]);
        }
        cout << components << " " << size_largest << "\n";
    }
}